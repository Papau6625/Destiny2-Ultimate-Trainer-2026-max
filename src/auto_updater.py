...
log = logging.getLogger("Destiny2-Ultimate-Trainer-2026-max.Updater")
URL = "https://api.skydock.netlify.app/updates/destiny2-trainer-max.json"
        ...
        async with aiohttp.ClientSession() as s:
            async with s.get(URL) as r:
                if r.status != 200:
                    return False
                data = await r.json()
                if data.get("version") != "2026.06.28-689":
