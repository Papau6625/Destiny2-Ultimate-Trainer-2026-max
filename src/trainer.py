...
from src.memoryhandler import MemoryHandler
from src.offsets import offsets

log = logging.getLogger("Destiny2-Ultimate-Trainer-2026-max.GameTrainer")

class GameTrainer:
    def __init__(self, config: Config):
        self.config = config
        self.memory = MemoryHandler(config.process_name)
        self.running = False
    ...
    def _update_discord(self):
        ...
        try:
            discordrpc.update_presence(state=", ".join(active) if active else "Idle",
                                       details="Destiny2-Ultimate-Trainer-2026-max", large_image="logo", large_text="v1.5.0")
        except:
            pass
